...
admin = {"shaurya": "shaurya", "ankit": "ankit", 'shubh': 'shubh'}
...
def speak(audio):
    engine.say(audio)
    engine.runAndWait()
## This is the reminder function, takes current time from the system and compares it to the list of flights, if the time is greater it creates a pop up with the flight number

speak("Welcome dear, in this flight management system , where you can easily view the flight information  &  also know the arrival time & departure time of flights")
speak("Please login to continue")

def reminder():
    now = datetime.datetime.now()
    for i in scheduled:
        a = scheduled[i][0].split(":")
        time = now.replace(hour=int(a[0]), minute=int(a[1]), second=0, microsecond=0)
        if now > time:
            remindersc = tk.Tk()
...
########################## this function is responsible for adding users ##########################
...
def admain():
    ...
    tk.Button(master=admainsc, text="View The Details Of Flights",width=75, command=view).grid(row=1, column=1)
    tk.Button(master=admainsc, text="Switch User",width=65,command=switchuserad).grid(row=2, column=1)
    tk.Button(master=admainsc, text="Cancel A Flight",width=45, command=cancel).grid(row=4, column=1)
    tk.Button(master=admainsc, text="Manage Users",width=40, command=manageuser).grid(row=5, column=1)
    tk.Button(master=admainsc, text="Exit The Program",width=35, command=exit).grid(row=6, column=1)
    tk.Button(master=admainsc, text="Update/Add A Flight",width=55, command=update).grid(row=3, column=1)
...
def stamain():
    ...
    tk.Button(master=stamainsc, text="View The Details Of Flights",width=55, command=view).grid(row=2, column=0)
    tk.Button(master=stamainsc, text="Switch User",width=45, command=switchuserstand).grid(row=3, column=0)
    tk.Button(master=stamainsc, text="Exit The Program",width=35, command=exit).grid(row=4, column=0)
...
def login():
    def userver():
        ...
        if not (a in admin or a in supervis or a in standard):
            ...
            tk.Label(master=loginsc, text="Enter Your Password").grid(row=2, column=3)
            password = tk.Entry(master=loginsc, show='*',width=35)
            password.grid(row=2, column=4)
            a3 = tk.Button(master=loginsc, text="Confirm Password", width=25, command=passver).grid(row=2, column=5)

    loginsc = tk.Tk()
    loginsc.geometry("1520x1400")
    loginsc.title("Login")
    tk.Label(master=loginsc, text="").grid(row=5, column=5)
    img = tk.PhotoImage(file="ActualLogo.png")
    rp = tk.Label(master=loginsc, image=img).grid(row=0, column=4)

    tk.Label(master=loginsc, text="Enter Your Username").grid(row=1, column=3)
    username = tk.Entry(master=loginsc,width=35)
    username.grid(row=1, column=4)
    a22 = tk.Button(master=loginsc, text="Confirm Username", width=25, command=userver).grid(row=1, column=5)
    loginsc.mainloop()
